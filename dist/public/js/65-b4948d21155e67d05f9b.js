...
/***/ function(module, exports, __webpack_require__) {
	...
	var actionsRedeem = __webpack_require__(759);
	var AddressInfo = __webpack_require__(775);
	...
	var RedeemScreen = React.createClass((_React$createClass = {
	  ...
	  mixins: [Reflux.listenTo(actionsRedeem.checkredeem.done, 'onCheckRedeemDoneAction'), Reflux.listenTo(actionsRedeem.checkredeem.error, 'onCheckRedeemErrorAction'), Reflux.listenTo(actions.getById.done, 'onGetByIdDoneAction'), Reflux.listenTo(actionsReceipt.getPaymentTerm.done, 'onGetPaymentTermDoneAction'), Reflux.listenTo(actionsReceipt.getHistoryPaymentTerm.done, 'onGetHistoryPaymentTermDoneAction'), Reflux.listenTo(actionsReceipt.printReceipt.done, 'onPrintReceiptDoneAction')],
	  ...
	  onCheckRedeemDoneAction: function onCheckRedeemDoneAction(data) {
	  ...
	  onCheckRedeemErrorAction: function onCheckRedeemErrorAction(data) {
	  ...
	  textEnter: function textEnter() {
	    if (this.state.contract.txt_code != '' && this.state.contract.barcode != '') {
	      var obj = {
	        code: this.state.contract.txt_code,
	        barcode: this.state.contract.barcode
	      };
	      actionsRedeem.checkredeem(obj);
	    } else {
	      this.state.disabledRedeem = true;
	      this.setState({
	        disabledRedeem: this.state.disabledRedeem
	      ...
	      toasterActions.pop({
	        type: 'warning',
	        message: 'กรุณากรอกเลขที่สัญญาและบาโค๊ดสินค้า'
	      });
	    }
	  },
	...
	}), _defineProperty(_React$createClass, 'doRedeem', function doRedeem() {
	  this.context.router.transitionTo('pos.receipt.screen', { id: this.state.nationid, contract_code: this.state.contract.code, redeem: 'Y' });
	  ...
	  return React.createElement(
	    'div',
	    { className: 'content-page layout-panel flex-form' },
	    React.createElement(
	      'div',
	      ...
	      React.createElement(
	        ...
	        React.createElement(FlexButton, { icon: 'forward19', label: 'pos.action.redeem.confirm',
	          'default': true,
