...
var CommissionDetail = React.createClass({
  ...
  getInitialState: function() {
    ...
    return {
      ...
      commissionPaymentFields: [
        ...
        {name:'num_receipt', label:'installment.commission-close.payment.num_receipt', width:32},
        ...
        {name:'status', label:'installment.commission-close.payment.status', width:72, render:function(row) {
      ...
      receiptFields: [
        ...
        {name:'no',label:'installment.commission-close.receipt.no',width:28,render:function(row,i) {
          return i+1;
        }},
        {name:'contract_code', label:'installment.commission-close.receipt.contract_code', width:150,render:function(row) {
          return row.contract_code;
        }},
        {name:'close_date', label:'installment.commission-close.receipt.close_date', width:100,render:function(row) {
          return tr.localize(new Date(row.close_date), {type:'date',format:'short'});
        }},
        {name:'sell_date', label:'installment.commission-close.receipt.sell_date', width:80, render:function(row) {
          return tr.localize(new Date(row.sell_date), {type:'date',format:'short'});
        }},
        {name:'code', label:'installment.commission-close.receipt.code', width:150,render:function(row) {
          return row.code;
        }},
        {name:'cus_name', label:'installment.commission-close.receipt.cus_name', width:120,render:function(row) {
          return row.cus_name;
        }},
        {name:'pay_date', label:'installment.commission-close.receipt.pay_date', width:100, render:function(row) {
          return tr.localize(new Date(row.pay_date), {type:'date',format:'short'});
        }},
        {name:'cost_term', label:'installment.commission-close.receipt.cost', width:100, className:'right', render: function(row) {
          return helper.numberFormat(row.cost_term, 2);
        }},
        {name:'amount', label:'installment.commission-close.receipt.amount', width:100, className:'right', render: function(row) {
          return helper.numberFormat(row.amount, 2);
        }},
        {name:'profit', label:'installment.commission-close.receipt.profit', width:100, className:'right', render: function(row) {
          return helper.numberFormat(row.profit, 2);
        }},
        {name:'display_name', label:'installment.commission-close.receipt.finance_name', width:120, render:function(row) {
  ...
  doVoid: function(comm_id) {
    dialogActions.show({
      title:'ยืนยัน',
      content: (
        <T content="installment.commission-close.confirm_to_void" component="div"/>
      ),
      actions: [
  ...
  doPaid: function(comm_id){
    dialogActions.show({
      title:'ยืนยัน',
      content: (
        <T content="installment.commission-close.confirm_to_paid" component="div"/>
      ),
      actions: [
        {id:'save', icon:'check52', label:'action.save'},
        {id:'cancel', icon:'close47', label:'action.cancel', default:true}
  ...
  render: function() {
    this.state.receiptFields[0].label = (<input type="checkbox" checked={this.state.chkAll} onChange={this.checkAll}/>);
    return (
      <div>
