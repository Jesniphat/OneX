...
var CustomerList = React.createClass({
  ...
  mixins:[
    ReFlux.listenTo(paymentTrackingAction.cancelBooking.done,'onCancelBookingDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.cancelBooking.error,'onCancelBookingErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.changePaidBooking.done,'onChangePaidBookingDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.changePaidBooking.error,'onChangePaidBookingErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.changePaidBookingList.done,'onChangePaidBookingListDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.changePaidBookingList.error,'onChangePaidBookingListErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.getDialogData.done,'onGetDialogDataDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.getDialogData.error,'onGetDialogDataDoneError'),
    ReFlux.listenTo(paymentTrackingAction.getDialogDetailList.done,'onGetDialogDetailListDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.getDialogItemList.done,'onGetDialogItemListDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.getDialogItemList.error,'onGetDialogItemListErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.getDialogDetailList.error,'onGetDialogDetailListErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.saveAddition.done,'onSaveAdditionDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.saveAddition.error,'onSaveAdditionErrorAction'),
    ReFlux.listenTo(paymentTrackingAction.sentDocToEmail.done,'onSentDocToEmailDoneAction'),
    ReFlux.listenTo(paymentTrackingAction.sentDocToEmail.error,'onSentDocToEmailErrorAction')
  ],
...
//    var shops = system.acl.getShopAcl();
    ...
    return {
      ...
      fields: [
        ...
        {name:'booking_date', title:'transport.booking_date'},//, width:'80px'
        {name:'total_amount', title:'transport.total_amount'},
        {name:'additional_amount', title:'transport.additional_amount'},
        {name:'status', title:'transport.status'},
        {name:'payment_status', title:'transport.paid_status'},
        {name:'receipt_type', title:'transport.receipt_type'},
        {name:'actions', type:'actions', width:(3*28+8)+'px', render:function(row) {
          var e = function() {
            this.dialogBookingOpen(row.id)
          }.bind(this);
          var g = function() {
            this.onChangePaid(row)
          }.bind(this);
          return (<div className="flex">
            ...
            <div onClick={g}>
              <FlexIcon icon="pinterest33" title="action.select"></FlexIcon>
            </div>
          </div>);
        }.bind(this)},
      ]
  ...
  onCancelBooking: function(row){
    ...
    dialogActions.show({
      ...
      console.log('DELETE');
      paymentTrackingAction.cancelBooking(row);
    });
  },

  onChangePaid: function(row) {
    console.log("onChangePaid = ", row);
    dialogActions.show({
      title:'preliminary.title.confirm_to_change_status',
      content:'Do you want to change status booking NO. ' + row.booking_no + ' to paid?',
      actions:[
        {id:'ok', icon:'check52', label:'dialog.confirm'},
        {id:'cancel', icon:'close47', label:'dialog.cancel', default:true}
      ]
      ...
      console.log('CHANGE');
      paymentTrackingAction.changePaidBooking(row);
    });
  },
  ...
  onChangePaidBookingDoneAction: function(data) {
      toasterActions.pop({
        type:'success',
        message:data.done
      });
  ...
  onChangePaidBookingErrorAction: function(errors) {
    console.log(errors);
      toasterActions.pop({
        type:'warning',
        message:'False to change status ' + errors
      });
  },

  onChangePaidBookingListDoneAction: function(data) {
    toasterActions.pop({
      type:'success',
      message:data.done
    });
  ...
  onChangePaidBookingListErrorAction: function(errors) {
    console.log(errors);
    toasterActions.pop({
      type:'warning',
      message:'False to change status '
    });
  },
  ...
  selectedRow: function(){
    ...
    var ch = this.refs.grid.checked();
    if(ch.length == 0){
      toasterActions.pop({
        type:'warning',
        message:"Don't have data to change status "
      });
      return;
    }

    dialogActions.show({
      title:'payment_tracking.dialogChangList',
      content:'Do you want to change status booking to paid?',
      actions:[
        {id:'ok', icon:'check52', label:'dialog.confirm'},
        {id:'cancel', icon:'close47', label:'dialog.cancel', default:true}
      ]
      ...
      paymentTrackingAction.changePaidBookingList(ch);
    });
  },
      ...
      <div className="layout-panel content-page">
        ...
        <div style={{minHeight:'540px',width:'1094px'}}>
            ...
            <div style={{height: '32px'}}>
                <div className="flaticon-been normal icon" style={{height: '32px',float: 'left'}}></div>
                <label style={{height:'32px',display: 'block',float: 'left',fontSize: '1.1em',paddingTop: '8px'}}>
                    <b>Payment</b>
                </label>
                <div onClick={this.selectedRow} style={{float:"right",marginRight:"25px"}}>
                  <FlexIcon icon="pinterest33" title="action.select"></FlexIcon>
                </div>
            </div>
              ...
              <div>
                <FlexGrid
                    ref="grid"
                    id="paymentTrackingBox"
                    listAction={paymentTrackingAction.listPayment}
                    exportAction={paymentTrackingAction.export}
