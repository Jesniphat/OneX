...
var intransitReceiptActions = require('./actions');
...
var PickupReceipt = React.createClass({
  ...
  mixins:[
    ...
    Reflux.listenTo(intransitReceiptActions.addBarcodeRecipt.done, 'onAddBarcodeDoneAction'),
    Reflux.listenTo(intransitReceiptActions.addBarcodeRecipt.error, 'onAddBarcodeErrorAction'),
    Reflux.listenTo(intransitReceiptActions.saveIntransitReceipt.done, 'onSaveIntransitReceiptAction'),
    Reflux.listenTo(intransitReceiptActions.saveIntransitReceipt.error, 'onSaveIntransitReceiptErrorAction')
  ],
  getInitialState: function() {
    ...
    this.intransitReceiptTable = [
      {name:'code',label:'intransitReceiptTable.pickup_no',width:'160px',render:function(row) {
        return (
          <div className="multiline">
            <div><span className="blue">{row.intransit_no}</span></div>
          </div>
        );
      }},
      {name:'product',label:'intransitReceiptTable.booking_no',render:function(row) {
        return (
          <div className="multiline" style={{whiteSpace:'normal'}}>{row.booking_no}</div>
        );
      }},
      {name:'serial',label:'intransitReceiptTable.item_no',width:'160px',render:function(row) {
        return (
          <div className="multiline">
      ...
      {name:'chk',label:(
          <span
            ...
            onClick={function() {me.removeAllFromReceipIntransit()}}
            />
        ),raw:true,width:'32px',render:function(row, i) {
        return (<span className="flaticon-clear5" onClick={function() {me.removeFromReceipIntransit(row, i)}}/>);
      }}
    ];
  ...
  onAddBarcodeDoneAction: function(res) {
    console.log(res.intransit_no);
    if (this.state.intransit_no==""){
      this.state.intransit_no = res.intransit_no;
      this.setState({
        intransit_no: this.state.intransit_no
      });
    }else if (this.state.intransit_no!=res.intransit_no){
      toasterActions.pop({
        type:'warning',
        message:"Intransit no not macht"
      });
      return;
    }
  ...
  onSaveIntransitReceiptAction: function(res) {
    console.log("save complete data = ", res);
    this.state.intransit_no = "";
    ...
    toasterActions.pop({
      type:'success',
      message:'Save intransit complete.'
    });
  },

  onSaveIntransitReceiptErrorAction: function(error){
    toasterActions.pop({
      type:'warning',
  ...
  handleEnter: function(id,value){
    ...
    intransitReceiptActions.addBarcodeRecipt({item_no:value});
  },

  removeFromReceipIntransit: function(row, i){
  ...
  removeAllFromReceipIntransit: function(){
    this.setState({
      puReceiptItems: []
    });
  },
  ...
  doSave: function(){
    ...
    intransitReceiptActions.saveIntransitReceipt(param);
  },

  render: function() {
    var intransitReceiptSummary = {
      count:0,
    };
    intransitReceiptSummary = this.state.puReceiptItems.reduce(function(prev, row) {
      return {
        count: prev.count+1
      };
    }, intransitReceiptSummary);

    this.state.receipt_qty = intransitReceiptSummary.count;

    var intransitReceiptFooter = (
      <tr>
        <td>Total</td>
        <td className="right"></td>
        <td className="right blue" style={{fontSize:'16px'}}>{intransitReceiptSummary.count} Item</td>
        <td></td>
      </tr>
    );
    return (
      ...
      <div className="box10 flex">
        <div className="box6">
          <div className="panel6" style={{borderLeft:'1px solid #eee',paddingLeft:'7px'}}>
            <FlexDataTable
              fields={this.intransitReceiptTable}
              data={this.state.puReceiptItems}
              key="stockin_id"
              displayRows={8}
              footer={intransitReceiptFooter}
            />
          </div>
        </div>
      </div>
