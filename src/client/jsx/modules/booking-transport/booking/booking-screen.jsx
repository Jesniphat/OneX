...
function disableDates(date) {
  ...
  return date < nowDate.setDate(nowDate.getDate() - 1);
}
...
var LoadFareScreen = React.createClass({
  ...
  mixins:[
    ...
    ReFlux.listenTo(actions.getPersonType.done, 'onGetPersonTypeDoneAction'),
    ReFlux.listenTo(actions.getPersonType.error, 'onGetPersonTypeErrorAction'),
    ReFlux.listenTo(actions.getPickUptime.done, 'onGetPickUptimeDoneAction'),
    ReFlux.listenTo(actions.getPickUptime.error, 'onGetPickUptimeErrorAction'),
    ReFlux.listenTo(actions.getCompanyProfile.done, 'onGetCompanyProfileDoneAction'),
    ReFlux.listenTo(actions.getCompanyProfile.error, 'onGetCompanyProfileErrorAction')
  ],

  getInitialState:function() {
    //console.log("getSession = ",system.sessionStore.getSession().staff);
    console.log("Props id = ", this.props.params.id);
    var prods_id = this.props.params.id;
    var staff_id = system.sessionStore.getSession().staff.id;
    if(this.props.params.id == 0 || this.props.params.id == '0'){
      ...
      console.log("staff side");
      var isPage = 1;
      var personId = this.props.params.id;
      var personType = "COMPANY";
      var isPaymentType = true;
    }
  ...
  componentDidMount:function() {
    ...
    if(this.state.prods_id != 0 || this.state.prods_id != '0'){
      console.log("staff side personId != 0");
      actions.getPersonType({person_id:this.state.personId});
    }
    ...
    actions.getPickUptime();
    actions.getRateType();
    actions.getCompanyProfile();
    this.setState({
  ...
  sumTotal:function() {
    ...
    var vatrate = 0 ;
    if(this.state.company_profile == null || this.state.company_profile == undefined || this.state.company_profile == ""){
      vatrate = 7;
    }else {
      vatrate = parseFloat(this.state.company_profile.vat_rate);
    }
    var total = 0;
    var itemQty = 0;
    var total_weight = 0;
    ...
    // total = total;
    console.log("total = ", total, " ", discountAmount);
    totalAmount = totalAmount + initialData.charge;
    console.log("totalAmount = ",totalAmount);
    vat_amount = ((totalAmount * vatrate)/100).toFixed(2);
    console.log("vat_amount = ", vat_amount);
    console.log("vat_rate = ", vatrate);
    grandTotalAmount = totalAmount + parseFloat(vat_amount);
  ...
  onGetPickUptimeDoneAction: function(listData){
    var pickUpdateGet = this.refs.picUpDate.getDate();
    var nowsDate = new Date();
    var cutTime = "non";
    if(pickUpdateGet.getMonth()==nowsDate.getMonth()&&pickUpdateGet.getDate()==nowsDate.getDate()){
      if((nowsDate.getHours()+2) >= 14){
        cutTime = "tomorrow";
      }else if ((nowsDate.getHours()+2) >= 10) {
        cutTime = "today";
      }
    }
    var list = listData.map(function(row) {
    ...
    for (var j=0; j<list.length; j++){
      if(cutTime == "non"){
        this.state.pickUpTimeList.value = "10.00 - 12.00";
        this.setState({
          pickUpTimeList:this.state.pickUpTimeList
        });
        timeList.push(<MenuItem key={list[j].value} value={list[j].value} primaryText={list[j].text} />);
      } else if (cutTime == "tomorrow") {
        this.state.pickUpTimeList.value = "";
        this.setState({
          pickUpTimeList:this.state.pickUpTimeList
        });
        timeList.push(<MenuItem key={list[j].value} value={list[j].value} primaryText={list[j].text} disabled={true}/>);
      } else if (cutTime == "today") {
        this.state.pickUpTimeList.value = "14.00 - 16.00";
        this.setState({
          pickUpTimeList:this.state.pickUpTimeList
        });
        if(list[j].value == "10.00 - 12.00"){
          timeList.push(<MenuItem key={list[j].value} value={list[j].value} primaryText={list[j].text} disabled={true}/>);
        }else {
          timeList.push(<MenuItem key={list[j].value} value={list[j].value} primaryText={list[j].text} />);
        }
    ...
    this.state.pickUpTimeList.list = timeList;
    this.setState({
      pickUpTimeList: this.state.pickUpTimeList
    })
  },
  ...
  onGetCompanyProfileDoneAction: function(profile){
    this.setState({
      company_profile: profile
    })
    console.log("company profile : ", this.state.company_profile);
  },

  onGetCompanyProfileErrorAction: function(error){
    console.log("company profile err = ", error);
  },
  ...
  handleChangePickUpTimeList:function(event, index, value){
    this.state.pickUpTimeList.value = value;
    this.setState({
      pickUpTimeList: this.state.pickUpTimeList
    });
    // this.checkDisabledTo();
  ...
  saveBooking:function() {
    ...
    var obj = {
      ...
      pickUpTime: this.state.pickUpTimeList.value,
      ...
      country_destination: this.state.rate.country_destination,
      staff_id: this.state.staff_id,
      vat_amount:this.state.vat_amount,
      vat_rate: this.state.vat_rate,
      grand_total_amount: this.state.grandTotalAmount,
      total: this.state.sumTotal
    };
  ...
  onSentMailDoneAction: function(data){
    ...
    if(this.refs.paymentType.getSelectedValue() == 'billing'){
      window.location.href='/signin-transport/#/finishbooking/' + data.bookingId + '/' + this.state.isPage;
    }else{
      window.location.href="http://localhost/temp_payment.php?bookingId=" + data.bookingId + "&amount=" + this.state.totalAmount;
    }
  },
  onSentMailErrorAction:function(data){
      console.log("Error can't sent email");
  },
  onGetPersonTypeDoneAction: function(res){
     console.log("res.type = ", res.type);
    if (res.type == "COMPANY") {
      this.state.isPaymentType = false;
    } else {
      this.state.isPaymentType = true;
    }
    this.setState({
      isPaymentType: this.state.isPaymentType
    });

    console.log("isPaymentType = ", this.state.isPaymentType);
  },
  onGetPersonTypeErrorAction: function(res){
    console.log("Error can't get type");
    toasterActions.pop({
      type:'warning',
      message:'Can not get person type'
    });
  },
  ...
  dialogAddBilling: function() {
    ...
    var c = this.refs;
    if(c.billingName.getValue() == "" || c.billingName.getValue() == null || c.billingName.getValue() == undefined){
      this.refs.billingName.focus();
      return;
    }
    if(c.billingAddr1.getValue() == "" || c.billingAddr1.getValue() == null || c.billingAddr1.getValue() == undefined){
      this.refs.billingAddr1.focus();
      return;
    }
    if(c.billingAddr2.getValue() == "" || c.billingAddr2.getValue() == null || c.billingAddr2.getValue() == undefined){
      this.refs.billingAddr2.focus();
      return;
    }
    if(c.billingTambon.getValue() == "" || c.billingTambon.getValue() == null || c.billingTambon.getValue() == undefined){
      this.refs.billingTambon.focus();
      return;
    }
    if(c.billingAmpher.getValue() == "" || c.billingAmpher.getValue() == null || c.billingAmpher.getValue() == undefined){
      this.refs.billingAmpher.focus();
      return;
    }
    if(c.billingProvince.getValue() == "" || c.billingProvince.getValue() == null || c.billingProvince.getValue() == undefined){
      this.refs.billingProvince.focus();
      return;
    }
    if(c.billingZipCode.getValue() == "" || c.billingZipCode.getValue() == null || c.billingZipCode.getValue() == undefined){
      this.refs.billingZipCode.focus();
      return;
    }
    if(c.billingTel.getValue() == "" || c.billingTel.getValue() == null || c.billingTel.getValue() == undefined){
      this.refs.billingTel.focus();
      return;
    }
    var billist = {
          ...
          >
            <div>
              ...
              <TextField
                ...
                floatingLabelText="NO./Building/Village"
              /><label style={{'margin-left':'20px'}}> </label>
              <TextField
                ref="billingAddr2"
                hintText="Alley/Road"
                floatingLabelText="Alley/Road"
              /><br/>
              <TextField
                ref="billingTambon"
                hintText="Tambon"
                floatingLabelText="Tambon"
              /><label style={{'margin-left':'20px'}}> </label>
              <TextField
                ref="billingAmpher"
                hintText="Amphur"
                floatingLabelText="Amphur"
              /><br/>
              <TextField
                ref="billingProvince"
                hintText="Province"
        ...
        <div className="main-box" style={{width: '985px', height: '602px', backgroundColor: '#F5F5F5', overflow: 'scroll', padding: '10px'}}>
          ...
          <div className="row">
            <div className="columns" style={{
                ...
                width: '957px'}}
                >
              <p style={{textAlign:'center',fontSize: '1.0em'}}>Get a quote and prepare a shipment</p>
            </div>
          </div>
              ...
              <div className="row1" style={{width:'100%'}}>
                ...
                <div className="large-10 columns1" style={{width:'92%',height:'72px',float: 'left',padding:'20px 15px 0px 15px'}}>
                  ...
                  <SelectField
                      ...
                      style={{fontSize:'1.0em', width:"403px"}}
                      hintText="Sender"
                    >
          ...
          <div style={this.state.disabledDatePick==true?{marginTop:'5px',border: 'solid 1px lightgray',backgroundColor:'#F5F5F5',width:'957px'}:{marginTop:'5px',border: 'solid 1px lightgray',backgroundColor:'#ffffff',width:'957px'}}>
            ...
            <div style={{marginTop:'10px',width:'100%',float:'left'}}>
              ...
              <div style={{width:'50%',float:'left'}}>
                  ...
                  <div className="large-10 columns2" style={{width:'92%',height:'32px',padding:'0px 15px',float:'left'}}>
                    <p style={{paddingTop:'10px'}}><b>Pickup Time</b></p>
                  </div>
              </div>
            </div>
            <div style={{marginTop:'10px',width:'100%',float:'left'}}>
              ...
              <div style={{width:'50%',float:'left'}}>
                ...
                <div className="large-10 columns2" style={{width:'92%',padding:'0px 19px',float:'left',height:'48px'}}>
                  <SelectField
                      ref="pickUpTime"
                      value={this.state.pickUpTimeList.value}
                      onChange={this.handleChangePickUpTimeList}
                      disabled={this.state.disabledDatePick}
                      style={{fontSize:'1.0em',height:"48px"}}
                      hintText="Pickup time"
                    >
                      {this.state.pickUpTimeList.list}
                  </SelectField>
                </div>
              </div>
            </div>
            ...
            <div style={{marginTop:'15px',padding:'0px 30px 0px 30px',width:'100%',float:'left'}}>
                <div style={this.state.checkDataTable > 0 ? {display:'block'} : {'display':'none'}}>
                  ...
                  >
                    ...
                    >
                      <TableRow>
                        <TableRowColumn style={{textAlign: 'left','width': '22%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>Sub total</TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{helper.numberFormat(this.state.sumTotal,2)}</TableRowColumn>
                      ...
                      <TableRow style={this.state.discount==0 ? {display:'none'} : {display:'table-row'}}>
                      ...
                      <TableRow>
                        <TableRowColumn style={{textAlign: 'left','width': '22%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{this.state.vatText}</TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{helper.numberFormat(parseInt(this.state.vat_amount),2)}</TableRowColumn>
                        <TableRowColumn style={{textAlign: 'center', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{this.state.currencyCodeTemp}</TableRowColumn>
                        <TableRowColumn style={{'min-width':'103px', 'width': '8%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                      </TableRow>
                      <TableRow>
                        <TableRowColumn style={{textAlign: 'left','width': '22%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>Grand total</TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                        <TableRowColumn style={{textAlign: 'right', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{helper.numberFormat(this.state.grandTotalAmount,2)}</TableRowColumn>
                        <TableRowColumn style={{textAlign: 'center', 'width': '10%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}>{this.state.currencyCodeTemp}</TableRowColumn>
                        <TableRowColumn style={{'min-width':'103px', 'width': '8%','padding':'0px','padding-left':'3px','vertical-align':'middle'}}></TableRowColumn>
                      </TableRow>
                    </TableFooter>
            ...
            <div style={{marginTop:'15px',padding:'0px 15px',float:'left',width:'100%'}}>
                ...
                I have read and agree to the <a href="http://www.w3schools.com"><u>Terms & Condition</u></a>
                </div>
            </div>
