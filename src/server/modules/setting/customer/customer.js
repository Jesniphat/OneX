...
router.post('/list', [bodyParser.json()], function(req, res) {
  var db = conn.connect();
  var $scope = {};
  var mainQuery = 'SELECT * FROM customer ' +
      ' WHERE is_active=\'YES\' ';
  var cond = [];
  ...
  var getCount = function() {
    var sql = "SELECT COUNT(*) AS cnt FROM customer WHERE is_active='YES'";
    if (cond.length > 0) {
...
router.post('/getById', [bodyParser.json()], function(req, res) {
  var db = conn.connect();
  var sql = "SELECT * FROM customer WHERE id=:id AND is_active='YES'";
  db.query(sql, {id:req.body.id}).then(function(rows){
    if (rows.length==0) {
      throw new Error('customer.error.not_found');
    }
    res.send({
      status: true,
      data: rows[0]
...
router.post('/save', [bodyParser.json()], function(req, res) {
  ...
  if (data.code == '') {
    error.push('customer.error.code_empty');
  }

  if (data.fullname == '') {
    error.push('customer.error.name_empty');
  }
  ...
  var checkDup = function() {
    var sql = "SELECT 1 AS cnt FROM customer WHERE code=:code AND id <> :id";
  ...
  var insertData = function() {
    var sql =  "INSERT INTO customer (code, fullname, nation_id, tax_id, is_active) "
      "VALUES (:code, :fullname, :nation_id, :tax_id, 'YES')";
  ...
  var updateData = function() {
    var sql = "UPDATE customer SET code=:code, fullname=:fullname, "
    + "nation_id=:nation_id, tax_id=:tax_id "
    + "WHERE id=:id";
...
router.post('/delete', [bodyParser.json()], function(req, res) {
  ...
  var deactiveData = function() {
    var sql = "UPDATE customer SET is_active='NO' WHERE id=:id";
    return db.query(sql, {id:id});
  }
