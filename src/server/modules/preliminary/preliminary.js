...
router.post('/saveCustomers',[bodyParser.json()], function(req, res) {
  ...
  var insertCustomers = function() {
    ...
    var insertDataBillingList = function(i) {
      ...
      if(req.body.billingListData[i].billingCode == ''||req.body.billingListData[i].billingCode == null||req.body.billingListData[i].billingCode == undefined){
            req.body.billingListData[i].billingCode = '';
        }
    ...
    var selectDefultBill = function(){
      console.log('selectDefultAddr  $scope.customer_id = ',$scope.customer_id);
      return q.all([
        (function() {
    ...
    var updateAddBillDefault = function(){
      ...
      console.log("check id2 = ");
      req.body.customer_id = $scope.customer_id;
      console.log("$scope.defultBillId = ",$scope.defultBillId)
      if($scope.defultBillId == undefined){
        req.body.defultBillId = '0';
      }else {
        console.log("check id1 = ");
        console.log($scope.defultBillId.id);
      ...
      oraConn.query(oradb, oraSql, item, true).then(function(){
        if(params.group_id !== "0") {
          db.delete('point_group_product', { point_group_id: params.group_id }).then(function(){
            ...
            oraConn.close(oradb);
            res.send({ status:true, id: data.insertId, code: params.group_code, name: params.group_name });
          }).catch(function(e){
            oraConn.close(oradb);
            res.send({ status:false, error: e });
          });
